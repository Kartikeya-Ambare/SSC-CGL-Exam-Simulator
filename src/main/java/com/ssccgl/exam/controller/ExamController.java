...
public class ExamController {
    ...
    public String attemptExam(@PathVariable Long sessionId,
        ...
        try {
            String examStateJson = objectMapper.writeValueAsString(examState);
            model.addAttribute("examStateJson", examStateJson);
        } catch (Exception e) {
            logger.error("Failed to serialize examState", e);
            model.addAttribute("examStateJson", "{}");
        }
        model.addAttribute("session", session);
        model.addAttribute("examState", examState);
        model.addAttribute("user", user);
